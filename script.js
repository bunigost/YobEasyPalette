...
function drawImage() {
    ...
    console.log(`Image drawn with dimensions: ${canvas.width}x${canvas.height}`); // Log dimensions
}
...
imageUploader.addEventListener('change', (event) => {
    ...
    if (file) {
        ...
        img = new Image();
        img.onload = () => {
            console.log(`Image loaded: ${img.width}x${img.height}`); // Log image dimensions
...
canvas.addEventListener('click', (event) => {
    ...
    const [r, g, b, a] = pixel;
    console.log(`Clicked pixel color: rgba(${r}, ${g}, ${b}, ${a})`);
...
function loadLayerImage(layerId, imageUrl, callback) {
    ...
    layerImg.src = `preview/${imageUrl}`; // Load image from combined path
}
...
function loadImagesForPreview(selectedPreview, callback) {
    ...
    images.forEach((layer, index) => {
        const imageUrl = `${selectedPreview}/${layer}`;
        console.log(`Loading image from: preview/${imageUrl}`); // Log for debugging
...
// Mario First Preview
    document.addEventListener("DOMContentLoaded", function() {
        // Get the Mario image element
        const marioImage = document.querySelector('img[alt="Mario"]');

        // Simulate a click event on the Mario image to select it
        if (marioImage) {
            marioImage.click(); // Trigger whatever click event you have set up for images.
        }
    });
